...
<footer class="flowdesk-gradient text-white mt-24">
	<div class="container-fd py-16 grid gap-12 md:grid-cols-3">
		<div>
			<p class="font-heading text-xl font-bold"><?php bloginfo( 'name' ); ?></p>
			<p class="mt-3 text-blue-100 text-sm max-w-xs">
		...
		<nav aria-label="<?php esc_attr_e( 'Navegación de footer', 'flowdesk' ); ?>">
			<p class="font-heading font-semibold mb-3"><?php esc_html_e( 'Producto', 'flowdesk' ); ?></p>
			<?php
			wp_nav_menu( array(
				'theme_location' => 'footer',
				'container'      => false,
				'menu_class'     => 'space-y-2 text-sm text-blue-100',
				'fallback_cb'    => false,
			) );
			?>
		</nav>
		...
		<div>
			<p class="font-heading font-semibold mb-3"><?php esc_html_e( 'Newsletter', 'flowdesk' ); ?></p>
			...
			<form class="fd-newsletter-form flex gap-2" novalidate>
				...
				<input
					...
					class="min-w-0 flex-1 rounded-lg px-3 py-2 text-slate-900 text-sm"
				/>
	...
	<div class="border-t border-white/20">
		<div class="container-fd py-6 text-sm text-blue-100 flex flex-col sm:flex-row justify-between gap-2">
